...
def get_range_hint(number, current_min=1, current_max=100):
   start = max(current_min, number - 12)
   end = min(current_max, number + 12)
   return f"HINT: The narrowed range around the number is {start}-{end}"
def get_thefirst_digit_hint(number):
   result = str(number)
   return f"HINT: The first digit of the number is {result[0]}"
...
while True:
   ...
   elif attempt >= 5 and attempt < 7:
       print(get_divisibility_hint(random_number))
   elif attempt >= 7 and attempt < 10:
